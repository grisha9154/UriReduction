...
function fileUploadReducer() {
    ...
    switch (action.type) {
        ...
        case actions.FILE_UPLOAD_PROGRESS:
            {
                var _action$payload = action.payload, _identificator2 = _action$payload.identificator, file = _action$payload.file, progress = _action$payload.progress, isImage = _action$payload.isImage, isDoc = _action$payload.isDoc;
                if (isImage)
                    return updateUploadingImage(state, _identificator2, file, function (uploadingImage) {
                        return uploadingImage.set('progress', Math.round(progress));
                    });
                if (isDoc)
                    return updateUploadingDocument(state, _identificator2, file, function (uploadingDocument) {
        ...
        case actions.FILE_UPLOAD_ERROR:
            {
                var _action$payload2 = action.payload, _identificator3 = _action$payload2.identificator, _file = _action$payload2.file, error = _action$payload2.error, _isImage = _action$payload2.isImage, _isDoc = _action$payload2.isDoc;
                if (_isImage)
                    return updateUploadingImage(state, _identificator3, _file, function (uploadingImage) {
                        return uploadingImage.set('error', error);
                    });
                if (_isDoc)
                    return updateUploadingDocument(state, _identificator3, _file, function (uploadingDocument) {
...
function updateUploadingImage(state, identificator, file, updater) {
    return state.updateIn(['images', identificator], function (images) {
        return (0, _immutable.List)(images).map(function (uploadingImage) {
            return (uploadingImage.file === file ? updater(uploadingImage) : uploadingImage);
        });
    });
}
